 ...
 * this scaffold accepts various microdata schema arguments to create Search Engine Optimized content.
...
$classnames = function ($name, $fmt = 'class="%"') use(&$config) {
    ...
    if (is_array($name)) {
        ...
        $name = '';
        array_walk($names, 
            function ($name) use(&$n, $classnames) {
                ob_start();
                $classnames($n, '%');
...
?><section <?php $schema('section');?> <?php $classnames('section');?>
	title="<?php echo htmlspecialchars($config['title']);?>">
	<header <?php $classnames('header');?>>
